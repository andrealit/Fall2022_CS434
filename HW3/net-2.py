...
def main():
  ...
  ################################
  x_size = len(X_test)
  rowN = np.arange(0,x_size)
  rowN = rowN[:,None]
  test_out = np.argmax(net.forward(X_test), axis=1)[:,np.newaxis]
  test_final = np.hstack((rowN, test_out))
  np.savetxt("test_prediction_hw3.csv", test_final, header="id,digit", fmt='%i', comments='', delimiter=",")
